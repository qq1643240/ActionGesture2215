...
- (NSString *)titleForShortcut:(NSString *)s { NSDictionary *m=@{AGShortcutOff:@"快捷动作：关闭",AGShortcutWeChatScan:@"微信扫码",AGShortcutWeChatPay:@"微信付款",AGShortcutAlipayScan:@"支付宝扫码",AGShortcutAlipayPay:@"支付宝付款"}; return m[s]?:m[AGShortcutOff]; }
...
- (BOOL)executeGesture:(NSString *)g onButton:(SBRingerHardwareButton *)button event:(id<AGHardwareButtonEvent>)event { if(!button||!event)return NO; AGGestureConfiguration *c=[self configurationForGesture:g]; if(!c){[self snapshotNativeConfigurationForGesture:g];c=[self configurationForGesture:g];} if(!c)return NO; /* Native action always wins. Shortcut runs only when native selection is 无操作 (no archive). */ if(!c.hasArchive){NSURL *url=[self shortcutURL:[self shortcutForGesture:g]];if(url){dispatch_async(dispatch_get_main_queue(),^{UIApplication *app=UIApplication.sharedApplication;if([app canOpenURL:url])[app openURL:url options:@{} completionHandler:nil];});return YES;}} NSString *identifier=g; BOOL selected=[self selectConfiguration:c identifier:identifier button:button]; if(!selected)selected=[self applyConfiguration:c]; return selected&&[self replayNativeActionOnButton:button event:event]; }
