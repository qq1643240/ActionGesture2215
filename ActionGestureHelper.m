...
- (NSString *)actionLogPath { NSMutableArray *paths=[NSMutableArray arrayWithObjects:@"/rootfs/var/tmp/ActionGesture2215.log",@"/var/tmp/ActionGesture2215.log",nil]; NSString *rootHidePath=jbroot(@"/rootfs/var/tmp/ActionGesture2215.log"); if(rootHidePath.length)[paths insertObject:rootHidePath atIndex:1]; NSFileManager *fm=NSFileManager.defaultManager; for(NSString *p in paths){NSString *dir=[p stringByDeletingLastPathComponent]; if(![fm fileExistsAtPath:dir]) [fm createDirectoryAtPath:dir withIntermediateDirectories:YES attributes:nil error:nil]; if([fm isWritableFileAtPath:dir]) return p;} return paths.lastObject; }
- (void)recordEvent:(NSString *)event { if(!event.length)return; NSString *line=[NSString stringWithFormat:@"%@ [pid=%d bundle=%@] %@\n",[NSDate date],getpid(),NSBundle.mainBundle.bundleIdentifier?:@"-",event]; NSString *path=[self actionLogPath]; NSFileHandle *f=[NSFileHandle fileHandleForWritingAtPath:path]; if(!f){[[line dataUsingEncoding:NSUTF8StringEncoding] writeToFile:path atomically:YES];return;} @try{[f seekToEndOfFile];[f writeData:[line dataUsingEncoding:NSUTF8StringEncoding]];[f closeFile];}@catch(__unused NSException *e){} }
...
- (void)systemActionPreferenceDidChangeForKey:(NSString *)key { if(_suppressSystemActionSnapshots)return; if(![key isEqual:@"SBSystemActionSelectedSectionIdentifier"]&&![key isEqual:@"SBSystemActionConfiguredActionArchive"])return; [self recordEvent:[NSString stringWithFormat:@"system setting changed key=%@ gesture=%@",key,_currentGesture]]; NSString *g=_currentGesture; dispatch_async(dispatch_get_main_queue(),^{ if(!self.snapshotScheduled){self.snapshotScheduled=YES; self.pendingSnapshotGesture=g; dispatch_async(dispatch_get_main_queue(),^{NSString *p=self.pendingSnapshotGesture;self.pendingSnapshotGesture=nil;self.snapshotScheduled=NO;[self snapshotNativeConfigurationForGesture:p];[self recordEvent:[NSString stringWithFormat:@"native configuration snapshotted gesture=%@",p]];});} }); }
