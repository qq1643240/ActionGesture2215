...
- (BOOL)canUseShortcutForGesture:(NSString *)g { AGGestureConfiguration *c=[self configurationForGesture:g]; if([g isEqual:_currentGesture]){AGGestureConfiguration *current=[self currentNativeConfiguration];if(current)c=current;} return !c||[self isNoNativeAction:c]; }
...
- (void)initializeConfigurationForGesture:(NSString *)g { if(![self isKnownGesture:g]||[self hasStoredConfigurationForGesture:g])return; AGGestureConfiguration *c=[AGGestureConfiguration new]; c.hasSection=NO;c.hasArchive=NO;[self storeConfiguration:c forGesture:g]; }
- (void)snapshotNativeConfigurationForGesture:(NSString *)g { AGGestureConfiguration *c=[self currentNativeConfiguration]; [self storeConfiguration:c forGesture:g]; if(c.hasSection&&c.sectionIdentifier.length)[self saveShortcut:AGShortcutOff forGesture:g]; }
...
- (BOOL)selectConfiguration:(AGGestureConfiguration *)c identifier:(NSString *)identifier button:(SBRingerHardwareButton *)button { SBSystemActionAbstractDataSource *ds=[self dataSourceForButton:button]; if(![ds respondsToSelector:@selector(setSelectedSystemAction:)])return NO; if(!c.hasArchive){[ds setSelectedSystemAction:nil];if([ds respondsToSelector:@selector(updateSelectedAction)])[ds updateSelectedAction];return YES;} SBLinkSystemAction *a=[self systemActionForConfiguration:c identifier:identifier];if(!a)return NO;[ds setSelectedSystemAction:a];if([ds respondsToSelector:@selector(updateSelectedAction)])[ds updateSelectedAction];return YES; }
