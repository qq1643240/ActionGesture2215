...
- (BOOL)canUseShortcutForGesture:(NSString *)g { AGGestureConfiguration *c=[self configurationForGesture:g]; return !c||!c.hasArchive; }
...
- (BOOL)isNoNativeAction:(AGGestureConfiguration *)c { return !c||!c.hasArchive||!c.configuredActionArchive.length; }
...
- (BOOL)selectConfiguration:(AGGestureConfiguration *)c identifier:(NSString *)identifier button:(SBRingerHardwareButton *)button { SBSystemActionAbstractDataSource *ds=[self dataSourceForButton:button]; if(![ds respondsToSelector:@selector(setSelectedSystemAction:)])return NO; if(!c.hasArchive||!c.configuredActionArchive){[ds setSelectedSystemAction:nil];return YES;} SBLinkSystemAction *a=[self systemActionForConfiguration:c identifier:identifier];if(!a)return NO;[ds setSelectedSystemAction:a];return YES; }
