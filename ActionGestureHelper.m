...
- (AGGestureConfiguration *)configurationForGesture:(NSString *)g;
@end
...
- (NSString *)shortcutForGesture:(NSString *)g { if(![self canUseShortcutForGesture:g])return AGShortcutOff; NSString *s=[self preferenceValueForKey:[NSString stringWithFormat:@"shortcut.%@",g]]; return [self isKnownShortcut:s]?s:AGShortcutOff; }
- (BOOL)canUseShortcutForGesture:(NSString *)g { AGGestureConfiguration *c=[self configurationForGesture:g]; return !c||[self isNoNativeAction:c]; }
- (void)saveShortcut:(NSString *)s forGesture:(NSString *)g { if([self isKnownGesture:g]&&[self isKnownShortcut:s]&&( [s isEqual:AGShortcutOff]||[self canUseShortcutForGesture:g])) {[self setPreferenceValue:s forKey:[NSString stringWithFormat:@"shortcut.%@",g]];[self sync];[self recordEvent:[NSString stringWithFormat:@"saved shortcut=%@ gesture=%@",s,g]];} }
...
- (BOOL)isNoNativeAction:(AGGestureConfiguration *)c { if(!c||!c.hasSection||!c.sectionIdentifier.length)return YES; NSString *s=c.sectionIdentifier.lowercaseString; return [s containsString:@"none"]||[s containsString:@"noaction"]||[s containsString:@"no_action"]||[s containsString:@"noop"]||[s containsString:@"disabled"]; }
...
- (BOOL)canHandleButton:(SBRingerHardwareButton *)button { if(!button||!_originalButtonDown||!_originalButtonLongPress||!_originalButtonUp)return NO; SBSystemActionAbstractDataSource *ds=[self dataSourceForButton:button]; return [ds respondsToSelector:@selector(setSelectedSystemAction:)]; }
- (NSUserDefaults *)springBoardDefaults { return [[NSUserDefaults alloc]initWithSuiteName:@"com.apple.springboard"]; }
- (AGGestureConfiguration *)currentNativeConfiguration { NSUserDefaults *d=[self springBoardDefaults]; NSString *section=[d objectForKey:@"SBSystemActionSelectedSectionIdentifier"]; NSData *archive=[d objectForKey:@"SBSystemActionConfiguredActionArchive"]; AGGestureConfiguration *c=[AGGestureConfiguration new]; c.hasSection=[section isKindOfClass:NSString.class]&&section.length>0; c.hasArchive=c.hasSection&&[archive isKindOfClass:NSData.class]&&archive.length>0; c.sectionIdentifier=c.hasSection?section:nil; c.configuredActionArchive=c.hasArchive?archive:nil; return c; }
...
- (void)initializeEmptyConfigurationForGesture:(NSString *)g { if(![self isKnownGesture:g]||[self hasStoredConfigurationForGesture:g])return; AGGestureConfiguration *c=[AGGestureConfiguration new]; c.hasSection=NO; c.hasArchive=NO; [self storeConfiguration:c forGesture:g]; [self saveShortcut:AGShortcutOff forGesture:g]; }
- (void)snapshotNativeConfigurationForGesture:(NSString *)g { AGGestureConfiguration *c=[self currentNativeConfiguration]; [self storeConfiguration:c forGesture:g]; if(c.hasArchive&&c.configuredActionArchive.length){[self saveShortcut:AGShortcutOff forGesture:g];} }
- (BOOL)applyConfiguration:(AGGestureConfiguration *)c { if(!c)return NO; NSUserDefaults *d=[self springBoardDefaults]; BOOL old=_suppressSystemActionSnapshots; _suppressSystemActionSnapshots=YES; if(c.hasSection&&c.sectionIdentifier.length)[d setObject:c.sectionIdentifier forKey:@"SBSystemActionSelectedSectionIdentifier"];else [d removeObjectForKey:@"SBSystemActionSelectedSectionIdentifier"]; if(c.hasSection&&c.hasArchive&&c.configuredActionArchive)[d setObject:c.configuredActionArchive forKey:@"SBSystemActionConfiguredActionArchive"];else [d removeObjectForKey:@"SBSystemActionConfiguredActionArchive"]; BOOL ok=[d synchronize]; _suppressSystemActionSnapshots=old; return ok; }
- (BOOL)applyNativeConfigurationForGesture:(NSString *)g { return [self applyConfiguration:[self configurationForGesture:g]]; }
- (BOOL)reloadSelectedActionOnButton:(SBRingerHardwareButton *)button { SBSystemActionAbstractDataSource *ds=[self dataSourceForButton:button]; if(![ds respondsToSelector:@selector(updateSelectedAction)])return NO; [ds updateSelectedAction]; return YES; }
...
- (BOOL)selectConfiguration:(AGGestureConfiguration *)c identifier:(NSString *)identifier button:(SBRingerHardwareButton *)button { SBSystemActionAbstractDataSource *ds=[self dataSourceForButton:button]; if(![ds respondsToSelector:@selector(setSelectedSystemAction:)])return NO; if([self isNoNativeAction:c]||!c.hasArchive||!c.configuredActionArchive){[ds setSelectedSystemAction:nil];return YES;} SBLinkSystemAction *a=[self systemActionForConfiguration:c identifier:identifier];if(!a)return NO;[ds setSelectedSystemAction:a];return YES; }
...
- (BOOL)executeGesture:(NSString *)g onButton:(SBRingerHardwareButton *)button event:(id<AGHardwareButtonEvent>)event { if(!button||!event)return NO; AGGestureConfiguration *c=[self configurationForGesture:g]; if(!c){[self snapshotNativeConfigurationForGesture:g];c=[self configurationForGesture:g];} if(!c)return NO; NSString *shortcut=[self shortcutForGesture:g]; BOOL noNative=[self isNoNativeAction:c]; [self recordEvent:[NSString stringWithFormat:@"execute gesture=%@ nativeState=%@ section=%@ archive=%@ shortcut=%@",g,noNative?@"none":@"present",c.sectionIdentifier?:@"-",c.hasArchive?@"present":@"none",shortcut]]; if(noNative){NSURL *url=[self shortcutURL:shortcut];if(url)return [self openShortcutURL:url];return NO;} NSString *identifier=g; BOOL selected=[self selectConfiguration:c identifier:identifier button:button]; if(!selected){BOOL applied=[self applyConfiguration:c];if(applied){__weak SBRingerHardwareButton *wb=button;dispatch_after(dispatch_time(DISPATCH_TIME_NOW,120*NSEC_PER_MSEC),dispatch_get_main_queue(),^{[self reloadSelectedActionOnButton:wb];});}selected=applied;} return selected&&[self replayNativeActionOnButton:button event:event]; }
